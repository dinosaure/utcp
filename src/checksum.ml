...
let unsafe_feed_16_le ?(off = 0) ~len:top sum buf =
  let len = ref top in
  let sum = ref sum in
  let i = ref 0 in
  while !len >= 2 do
...
let unsafe_feed_16_be ?(off = 0) ~len:top sum buf =
  let len = ref top in
  let sum = ref sum in
  let i = ref 0 in
  while !len >= 2 do
...
let feed_string ?(off= 0) ?len sum str =
  ...
  if Sys.big_endian
  then unsafe_feed_16_be ~off ~len sum str
  else unsafe_feed_16_le ~off ~len sum str
...
let digest_string ?(off= 0) ?len str =
  ...
  let sum =
    if Sys.big_endian
    then unsafe_feed_16_be ~off ~len 0 str
    else unsafe_feed_16_le ~off ~len 0 str in
  finally sum

let digest_strings sstr =
  let fn = match Sys.big_endian with
    | true -> fun sum str ->
        unsafe_feed_16_be ~off:0 ~len:(String.length str) sum str
    | false -> fun sum str ->
        unsafe_feed_16_le ~off:0 ~len:(String.length str) sum str in
